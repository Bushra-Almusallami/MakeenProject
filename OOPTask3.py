from datetime import date
class Person:
    def __init__(self,name,age,address,hours):
        self.name=name
        self.age=age
        self.address=address
        self.hours=hours
        
    def class_type(self):
        return "This is parent class"
    
class Student(Person): # Student is a child of Person
    def __init__(self,name,age,address,hours,grade):
        super().__init__(name,age,address,hours)
        self.grade= grade
        
    def class_type(self):
        return "This is student class"
    
    @classmethod
    def new_student(cls,name,birth_year,address,hours,grade):
        return cls(name,date.today().year-birth_year,address,hours,grade)

s1= Student("Salim",23,"Muscat",180,[3.1,3.5,2])
p1= Person("Salim",23,"Muscat",180)
p2= Person("Bushra",23,"Muscat",180)
#print(p1.class_type())
#print(s1.class_type())
#print(type(s1))

s2=Student.new_student("Bushra",1998,"Muscat",180,[3.1,3.5,2])
print(date.today())
print(s2.age)

